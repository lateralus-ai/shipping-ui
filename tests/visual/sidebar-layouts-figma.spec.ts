...
import {
  FIGMA_BASELINE_DIR,
  FIGMA_SIDEBAR_LAYOUTS_ACTUAL_IMAGE,
  FIGMA_SIDEBAR_LAYOUTS_BASELINE_IMAGE,
  FIGMA_SIDEBAR_LAYOUTS_DIFF_IMAGE,
  FIGMA_SIDEBAR_LAYOUTS_FAILURES_JSON,
  FIGMA_SIDEBAR_LAYOUTS_GRID_CROP,
  FIGMA_SIDEBAR_LAYOUTS_MASK_IMAGE,
  FIGMA_SIDEBAR_LAYOUTS_REPORT_JSON,
  SIDEBAR_LAYOUTS_STORY,
} from "./figma.config";
import {
  compareFigmaRegions,
  formatDiffSummary,
...
import {
  SIDEBAR_LAYOUTS_COMPARISON_REGIONS,
  SIDEBAR_LAYOUTS_COMPARE_THRESHOLD,
  SIDEBAR_LAYOUTS_MAX_PER_REGION_DIFF_RATIO,
} from "./helpers/sidebar-layouts-comparison-regions";
...
const baselinePath = path.join(process.cwd(), FIGMA_SIDEBAR_LAYOUTS_BASELINE_IMAGE);
...
const saveReport = (result: CompareFigmaRegionsResult) => {
  ...
  const report = {
    ...
    story: SIDEBAR_LAYOUTS_STORY.id,
    baseline: FIGMA_SIDEBAR_LAYOUTS_BASELINE_IMAGE,
    actual: FIGMA_SIDEBAR_LAYOUTS_ACTUAL_IMAGE,
    diff: FIGMA_SIDEBAR_LAYOUTS_DIFF_IMAGE,
    mask: FIGMA_SIDEBAR_LAYOUTS_MASK_IMAGE,
    width: result.width,
    height: result.height,
    compareThreshold: SIDEBAR_LAYOUTS_COMPARE_THRESHOLD,
    compareMode: "box",
  ...
  writeFileSync(
    path.join(process.cwd(), FIGMA_SIDEBAR_LAYOUTS_REPORT_JSON),
    `${JSON.stringify(report, null, 2)}\n`,
  );

  writeFileSync(
    path.join(process.cwd(), FIGMA_SIDEBAR_LAYOUTS_FAILURES_JSON),
    `${JSON.stringify(
      {
...
const saveImages = (screenshot: Buffer, result: CompareFigmaRegionsResult) => {
  writeFileSync(
    path.join(process.cwd(), FIGMA_SIDEBAR_LAYOUTS_ACTUAL_IMAGE),
    screenshot,
  );
  writeFileSync(
    path.join(process.cwd(), FIGMA_SIDEBAR_LAYOUTS_DIFF_IMAGE),
    result.diffPng,
  );
  writeFileSync(
    path.join(process.cwd(), FIGMA_SIDEBAR_LAYOUTS_MASK_IMAGE),
    result.maskPng,
  );
};

const logComparison = (result: CompareFigmaRegionsResult) => {
  console.log("\nSidebar layouts vs Figma visual comparison:");
  console.log(
  ...
  console.log(`  Baseline:  ${FIGMA_SIDEBAR_LAYOUTS_BASELINE_IMAGE}`);
  console.log(`  Actual:    ${FIGMA_SIDEBAR_LAYOUTS_ACTUAL_IMAGE}`);
  console.log(`  Diff map:  ${FIGMA_SIDEBAR_LAYOUTS_DIFF_IMAGE}`);
  console.log(`  Mask:      ${FIGMA_SIDEBAR_LAYOUTS_MASK_IMAGE}`);
  console.log(`  Report:    ${FIGMA_SIDEBAR_LAYOUTS_REPORT_JSON}`);
  console.log(`  Failures:  ${FIGMA_SIDEBAR_LAYOUTS_FAILURES_JSON}\n`);
};

test.describe("Sidebar layouts vs Figma baseline", () => {
  test.beforeAll(() => {
    if (!existsSync(baselinePath)) {
      throw new Error(
        `Missing Figma baseline at ${FIGMA_SIDEBAR_LAYOUTS_BASELINE_IMAGE}. Run: npm run figma:baseline:sidebar:layouts`,
      );
    }
  });

  test("Storybook Sidebar Layouts canvas matches Figma layout grid", async ({ page }) => {
    const { width, height } = FIGMA_SIDEBAR_LAYOUTS_GRID_CROP;

    await loadStoryInIframe(page, SIDEBAR_LAYOUTS_STORY.id, {
      width: width + VIEWPORT_PADDING,
      height: height + VIEWPORT_PADDING,
    });

    const grid = page.locator("[data-figma-sidebar-layouts-grid]").first();
    ...
    const result = compareFigmaRegions(
      baseline,
      screenshot,
      SIDEBAR_LAYOUTS_COMPARISON_REGIONS,
      {
        threshold: SIDEBAR_LAYOUTS_COMPARE_THRESHOLD,
        perRegionMaxDiffRatio: SIDEBAR_LAYOUTS_MAX_PER_REGION_DIFF_RATIO,
      },
    );
